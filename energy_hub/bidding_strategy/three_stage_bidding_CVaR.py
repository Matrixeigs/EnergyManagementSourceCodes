"""
Three stage bidding strategy with CVaR constraints
...
def main(N_scenario_first_stage=100, N_scenario_second_stage=1000):
    ...
    temprature_relax_positive = {}
    beta_relax = {}

    epison = model.addVar(vtype=GRB.CONTINUOUS, name="epison")
    # Define the recovery index
    for i in range(N_scenario_first_stage):
        beta_relax[i] = model.addVar(vtype=GRB.CONTINUOUS, lb=0, name="beta_relax{0}".format(i))  # CVaR constraints
    ...
    for i in range(T):  # Dispatch at each time slot
        for j in range(N_scenario_second_stage):  # Scheduling plan under second stage plan
            for k in range(N_scenario_first_stage):  # Dispatch at each time slot
                ...
                temprature_in[i, j, k] = model.addVar(lb=temprature_in_min, ub=temprature_in_max * 2,
                                                      name="temprature_in{0}".format(
                                                          i * N_scenario_first_stage * N_scenario_second_stage + j * N_scenario_first_stage + k))
                temprature_relax_positive[i, j, k] = model.addVar(lb=0,
                                                                  name="temprature_relax_positive{0}".format(
                                                                      i * N_scenario_first_stage * N_scenario_second_stage + j * N_scenario_first_stage + k))
    ...
    for k in range(N_scenario_first_stage):
        for j in range(N_scenario_second_stage):
            for i in range(T):
                ...
                model.addConstr(
                    temprature_in[i, j, k] - temprature_in_max <= temprature_relax_positive[i, j, k])

    for k in range(N_scenario_first_stage):
        for i in range(T):
            for j in range(N_scenario_second_stage):
                model.addConstr(temprature_relax_positive[i, j, k]-epison <= beta_relax[k])

    obj_CVaR = epison
    for i in range(N_scenario_first_stage):
        obj_CVaR += weight_first_stage[i][0] * beta_relax[i] / (1 - confidential_level)
    ...
    for i in range(T):
        for j in range(N_scenario_first_stage):
            obj_DA += pDA[i, j] * Price_DA[i, j] * weight_first_stage[j]
    # Calculate the CVaR
    Beta = zeros(N_scenario_first_stage)
    for i in range(N_scenario_first_stage):
        Beta[i] = model.getVarByName("beta_relax{0}".format(i)).X
    ...
    Temprature_relax = zeros((T, N_scenario_second_stage, N_scenario_first_stage))
        ...
        for j in range(N_scenario_second_stage):  # Scheduling plan under second stage plan
            for k in range(N_scenario_first_stage):  # Dispatch at each time slot
                ...
                Temprature_in[i, j, k] = model.getVarByName("temprature_in{0}".format(
                    i * N_scenario_first_stage * N_scenario_second_stage + j * N_scenario_first_stage + k)).X
                Temprature_relax[i, j, k] = model.getVarByName("temprature_relax_positive{0}".format(
                    i * N_scenario_first_stage * N_scenario_second_stage + j * N_scenario_first_stage + k)).X
    Relax = zeros(N_scenario_first_stage)

    for k in range(N_scenario_first_stage):
        for i in range(T):
            for j in range(N_scenario_second_stage):
                Relax[k] += Temprature_relax[i, j, k] * weight_second_stage[j]
